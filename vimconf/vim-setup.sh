...
function make_link_cli(){
  if [ ! -e ${HOME}/.vim ]   ; then
    ln -s ${SCRIPT_DIR}/vim ${HOME}/.vim \
      && echo "vim conf dir Set OK!"
  fi

  if [ ! -e ${HOME}/.vimrc  -a $( echo ${VIMVERSION} < 7.4 | bc ) -eq 1 ] ; then
...
function make_link_gui(){
  if [ ! -e ${HOME}/.gvimrc -a $( echo ${VIMVERSION} < 7.4 | bc ) -eq 1 ] ; then
    ln -s ${SCRIPT_DIR}/gvimrc ${HOME}/.gvimrc \
      && echo "gvimrc Set OK!"
  fi
}
